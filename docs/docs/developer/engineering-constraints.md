...
### The release setup build must compile kani-cli exactly as `dist build` does

**Constraint.** The release job runs `kani-cli setup` before `dist build`. Cargo reuses that
compile only when profile, target triple and `RUSTFLAGS` all match dist's own kani-cli build:
`--profile dist`, an explicit `--target`, and `-Ctarget-feature=+crt-static` on MSVC, which dist
applies because `msvc-crt-static` defaults to true.

**Evidence.** Under `cargo run --release` the setup compile took 8m 43s on Linux and 16m 44s on
Intel macOS in the rc.3 release run, and dist then rebuilt kani-cli from scratch. With matching
flags, dist's kani-cli build finished in 0.30 to 1.26 seconds, and the build jobs fell from 32 to
21 minutes on Linux, 38 to 27 on Apple Silicon, 50 to 36 on Windows and 71 to 58.5 on Intel macOS.

**Consequence.** A mismatch costs a full kani-cli compile per runner without failing anything.

**Enforcement.** `fragments/build-setup.yml` and `release.yml` carry the same step; `release.yml`
is `allow-dirty` for dist, so both files are edited together.

**Revalidate when.** dist changes how it invokes cargo, `msvc-crt-static` is set, or the release
matrix gains a target that needs its own flags.

### Release packages must resolve identical dependency features

**Constraint.** `precise-builds = true` makes dist build kani-cli and kani-web in separate cargo
invocations, and each resolves features only for its own dependency graph. The `workspace-hack`
crate, generated by `cargo hakari`, pins the union of features so both invocations compile shared
dependencies once and ship the feature set that the default-members build in CI tests.
...
`cargo hakari generate` is rerun, including on Dependabot pull requests.

**Enforcement.** `ci.yml` runs `cargo hakari generate --diff` and `cargo hakari manage-deps
--dry-run`. `kani-shared` and the WASM-only members are traversal-excluded in
`.config/hakari.toml`, because extensions compile `kani-shared` for `wasm32` and must not inherit
native features. cargo-machete skips the generated crate through `workspace-hack/.ignore`.

**Revalidate when.** dist gains grouped package builds, Cargo stabilises `feature-unification`, a
native workspace member is added, or the Dockerfile changes its build invocations.
